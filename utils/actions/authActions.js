...
export const signUp = (firstName, lastName, email, password) => {
	// signUp returns an async function that serves as a redux action
	return async (dispatch) => {
		...
		try {
			...
			// Invoke the redux authenticate action: update redux state with token and userData
...
// Logout user by clear local storage and redux auth state
export const userLogout = (userData) => {
	return async (dispatch) => {
		...
		AsyncStorage.clear();
		clearTimeout(timer);
		// Invoke the redux logout action: clear redux auth state
		dispatch(logout());
	};
};
...
const createUser = async (firstName, lastName, email, userId) => {
	...
	const childRef = child(dbRef, `users/${userId}`); // Where to store the data
	await set(childRef, userData); // TODO -> Handle errors
	return userData;
};
...
// Save token, userId and expiryDate to local storage
